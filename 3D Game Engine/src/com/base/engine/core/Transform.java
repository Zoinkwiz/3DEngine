...
public class Transform {
	...
	public Transform() {
		...
		parentMatrix = new Matrix4f().initIndentity();
	}
	
	public void update() {
		if(oldPos == null) {

			oldPos = new Vector3f(pos.getX(), pos.getY(), pos.getZ());
			oldRot = new Quaternion(rot.getX(), rot.getY(), rot.getZ(), rot.getW());
			oldScale = new Vector3f(scale.getX(), scale.getY(), scale.getZ());
		} else {
	...
	public void rotate(Vector3f axis, float angle){
		rot = new Quaternion(axis, angle).mul(rot).normalised();
	}
